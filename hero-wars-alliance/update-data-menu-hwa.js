...
// Receber a pasta (relativa a hero-wars-alliance) e o prefixo como argumentos (ex.: menu-hwa calendar-hwa)
const folder = process.argv[2];
const prefix = process.argv[3];
if (!folder || !prefix) {
  console.error('Uso: node update-data-menu-hwa.js <pasta-relativa> <prefixo>');
  console.error('Exemplo: node update-data-menu-hwa.js menu-hwa calendar-hwa');
  process.exit(1);
}
...
// Caminho para o sitemap.xml (agora sitemap-hwa.xml na raiz)
const sitemapFile = path.join(__dirname, '..', 'sitemap-hwa.xml');
...
function updateSitemap(newDate) {
  ...
  pages.forEach(page => {
    const url = `https://alexandregames.com/hero-wars-alliance/${folder}/${page}`;
...
console.log('Atualizado: sitemap-hwa.xml');

// Atualizar o sitemap-index.xml
