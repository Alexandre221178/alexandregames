# lang_checker_hwa_fix_teams.py
# Programa para corrigir automaticamente traduções e nomes de equipes em arquivos de Hero Wars Alliance
    ...
    try:
        ...
        for team_list in team_matches:
            ...
            for team in team_names:
                heroes_in_team = [h.strip() for h in re.split(r'[、,]', team)]
                for hero_name in heroes_in_team:
                    ...
                    if not found:
                        if fix_teams:
                            # Try to find the English name and replace with correct translation
                            correct_name = None
                            for hero_key, trans_dict in heroes_data.items():
                                if isinstance(trans_dict, dict) and 'en' in trans_dict and trans_dict['en'] == hero_name:
                                    if lang in trans_dict:
                                        correct_name = trans_dict[lang]
                                        break
                            if correct_name and correct_name != hero_name:
                                # Add to fixes
                                if filepath not in fixes:
                                    fixes[filepath] = []
                                fixes[filepath].append((hero_name, correct_name))
                                issues.append(f"{filepath}: Corrigindo '{hero_name}' para '{correct_name}' na equipe em {lang}")
                            else:
                                # For ja, try closest match
                                if lang == 'ja':
                                    best_match = None
                                    best_sim = 0
                                    for hero_key, trans_dict in heroes_data.items():
                                    ...
                                    if best_sim > 0.0:
                                        ...
                                        issues.append(f"{filepath}: Nome de herói/titã '{hero_name}' na equipe não encontrado no JSON em {lang}")
                                else:
...
def main():
    ...
    if all_issues:
        print("Problemas encontrados e correções aplicadas:")
