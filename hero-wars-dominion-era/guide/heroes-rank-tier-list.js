/* Rankings live in heroes-rank-tier-list.json; array order is display order.
 * Portraits and available guides live in heroes-rank-tier-list-heroes.json.
...
(() => {
  ...
  const root = document.getElementById('hwde-tier-list');
  if (!root) return;
  ...
  function render(rankings, catalog) {
    ...
    const nextRows = tiers.map((tier, index) => {
      const row = element('section', 'hwde-tier hwde-tier-' + index);
      row.id = 'rank-' + tier.toLowerCase().replace('+', '-plus');
      const heading = element('h2', 'hwde-tier-label', tier);
      heading.id = row.id + '-title';
      row.setAttribute('aria-labelledby', heading.id);
      const body = element('div', 'hwde-tier-body');
      const count = element('span', 'hwde-tier-count');
      count.setAttribute('aria-label', root.dataset.countLabel);
      heading.append(count);
      const list = element('ul', 'hwde-hero-grid');
      const cards = rankings[tier].map(name => {
        ...
        const node = element('li', 'hwde-hero');
        ...
        const card = element(guide ? 'a' : 'div', 'hwde-hero-card');
        if (guide) {
        ...
        if (item.image) {
          ...
          img.loading = 'lazy';
          img.decoding = 'async';
          img.addEventListener('error', () => img.replaceWith(element('span', 'hwde-hero-initial', name[0])), { once: true });
          card.append(img);
        } else {
          card.append(element('span', 'hwde-hero-initial', name[0]));
        }
        const label = element('strong', 'hwde-hero-name');
        if (language === 'ja' && item.names?.ja) {
          label.append(element('span', '', item.names.ja), element('br'));
        }
        label.append(element('span', '', (language === 'ja' ? name : (item.names?.[language] || name)) + (fallback ? '(EN)' : '')));
        card.append(label);
        node.append(card);
        list.append(node);
        return { node, name: name + ' ' + (item.names?.[language] || '') };
      });
      const empty = element('p', 'hwde-tier-empty', root.dataset.empty);
  ...
  async function load() {
    ...
    try {
      const results = await Promise.all(['heroes-rank-tier-list.json', 'heroes-rank-tier-list-heroes.json'].map(async file => {
