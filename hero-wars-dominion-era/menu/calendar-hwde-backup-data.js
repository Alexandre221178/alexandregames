/* Calendar data for Hero Wars: Dominion Era
   - Keep links per language empty when you want to fill them manually
   - Image and paths are relative to the page that includes the calendar
   - Supports: en, pt, ja, es, fr, de
*/
window.CALENDAR_DATA = [
// ===================== FEB 2026 =====================
{
    weekday: "",
    date: "",
    image: "",
    alt: "",
  noStrip: true,
  extra: [
    {
        titles: { en: "Alexandre Games", pt: "Alexandre Games", de: "Alexandre Games", es: "Alexandre Games", fr: "Alexandre Games", ja: "Alexandre Games" },
        labelOnly: true
      },
  ]
  },

  // ===================== JANUARY 2026 =====================
  {
    weekday: "Thursday",
    date: "Jan, 01",
    image: "../../hero-wars-dominion-era/images/events/hero-tournament-of-power/hero-tournament-of-power-150px.webp",
    alt: "Tournament of Hero Power",
    titles: { 
      en: "Tournament of Hero Power", 
      pt: "Torneio de Poder do Herói", 
      de: "Helden-Kraftturnier", 
      es: "Torneo de Poder del Héroe", 
      fr: "Tournoi de Puissance des Héros", 
      ja: "ヒーローパワートーナメント" 
    },
    links: { 
      en: "../../hero-wars-dominion-era/events/hero-tournament-of-power-en.html", 
      pt: "../../hero-wars-dominion-era/events/hero-tournament-of-power-pt.html", 
      de: "", es: "", fr: "", ja: "" 
    },
    noStrip: true,
    extra: [
      {
        titles: { 
          en: "Elemental Synergy", 
          pt: "Sinergia Elemental", 
          de: "Elementare Synergie", 
          es: "Sinergia Elemental", 
          fr: "Synergie Élémentaire", 
          ja: "エレメンタルシナジー" 
        },
        links: { 
          en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", 
          pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", 
          de: "", es: "", fr: "", ja: "" 
        },
        noStrip: true
      },
      {
        titles: { 
          en: "Happy New Year! Thank you for following Alexandre Games!", 
          pt: "Feliz Ano Novo! Obrigado por acompanhar o Alexandre Games!", 
          de: "Frohes neues Jahr! Danke fürs Folgen!", 
          es: "¡Feliz Año Nuevo! ¡Gracias por seguir Alexandre Games!", 
          fr: "Bonne année ! Merci de suivre Alexandre Games !", 
          ja: "新年おめでとう！Alexandre Gamesをフォローしてくれてありがとう！" 
        },
        labelOnly: true
      }
    ]
  },
  {
    weekday: "Friday",
    date: "Jan, 02",
    image: "../../hero-wars-dominion-era/images/events/legacy-of-great-ones/legacy-of-great-ones-150px.webp",
    alt: "Legacy of Great Ones",
    titles: { 
      en: "Legacy of Great Ones", 
      pt: "Legado dos Grandes", 
      de: "Vermächtnis der Großen", 
      es: "Legado de los Grandes", 
      fr: "Héritage des Grands", 
      ja: "偉大なる者の遺産" 
    },
    links: { 
      en: "../../hero-wars-dominion-era/events/legacy-of-great-ones-en.html", 
      pt: "../../hero-wars-dominion-era/events/legacy-of-great-ones-pt.html", 
      de: "", es: "", fr: "", ja: "" 
    },
    noStrip: true,
    extra: [
      {
        titles: { 
          en: "Tournament of Hero Power", 
          pt: "Torneio de Poder do Herói", 
          de: "Helden-Kraftturnier", 
          es: "Torneo de Poder del Héroe", 
          fr: "Tournoi de Puissance des Héros", 
          ja: "ヒーローパワートーナメント" 
        },
        links: { 
          en: "../../hero-wars-dominion-era/events/hero-tournament-of-power-en.html", 
          pt: "../../hero-wars-dominion-era/events/hero-tournament-of-power-pt.html", 
          de: "", es: "", fr: "", ja: "" 
        },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      }
    ]
  },
  {
    weekday: "Saturday",
    date: "Jan, 03",
    image: "../../hero-wars-dominion-era/images/events/foundations-of-power/foundation-of-power-150px.webp",
    alt: "Foundations of Power",
    titles: { 
      en: "Foundations of Power", 
      pt: "Fundamentos do Poder", 
      de: "Grundlagen der Macht", 
      es: "Fundamentos del Poder", 
      fr: "Fondements de la Puissance", 
      ja: "パワーの基盤" 
    },
    links: { 
      en: "../../hero-wars-dominion-era/events/foundations-of-power-en.html", 
      pt: "../../hero-wars-dominion-era/events/foundations-of-power-pt.html", 
      de: "", es: "", fr: "", ja: "" 
    },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Hero Power", pt: "Torneio de Poder do Herói", de: "Helden-Kraftturnier", es: "Torneo de Poder del Héroe", fr: "Tournoi de Puissance des Héros", ja: "ヒーローパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/hero-tournament-of-power-en.html", pt: "../../hero-wars-dominion-era/events/hero-tournament-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Legacy of Great Ones", pt: "Legado dos Grandes", de: "Vermächtnis der Großen", es: "Legado de los Grandes", fr: "Héritage des Grands", ja: "偉大なる者の遺産" },
        links: { en: "../../hero-wars-dominion-era/events/legacy-of-great-ones-en.html", pt: "../../hero-wars-dominion-era/events/legacy-of-great-ones-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      }
    ]
  },
  {
    weekday: "Sunday",
    date: "Jan, 04",
    image: "../../hero-wars-dominion-era/images/events/foundations-of-power/foundation-of-power-150px.webp",
    alt: "Foundations of Power",
    titles: { en: "Foundations of Power", pt: "Fundamentos do Poder", de: "Grundlagen der Macht", es: "Fundamentos del Poder", fr: "Fondements de la Puissance", ja: "パワーの基盤" },
    links: { en: "../../hero-wars-dominion-era/events/foundations-of-power-en.html", pt: "../../hero-wars-dominion-era/events/foundations-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Hero Power", pt: "Torneio de Poder do Herói", de: "Helden-Kraftturnier", es: "Torneo de Poder del Héroe", fr: "Tournoi de Puissance des Héros", ja: "ヒーローパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/hero-tournament-of-power-en.html", pt: "../../hero-wars-dominion-era/events/hero-tournament-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Legacy of Great Ones", pt: "Legado dos Grandes", de: "Vermächtnis der Großen", es: "Legado de los Grandes", fr: "Héritage des Grands", ja: "偉大なる者の遺産" },
        links: { en: "../../hero-wars-dominion-era/events/legacy-of-great-ones-en.html", pt: "../../hero-wars-dominion-era/events/legacy-of-great-ones-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Secret Wealth", pt: "Riqueza Secreta", de: "Geheimer Reichtum", es: "Riqueza Secreta", fr: "Richesse Secrète", ja: "秘密の富" },
        links: { en: "#section3", pt: "#section3", de: "#section3", es: "#section3", fr: "#section3", ja: "#section3" }
      }
    ]
  },
  {
    weekday: "Monday",
    date: "Jan, 05",
    image: "../../hero-wars-dominion-era/images/events/foundations-of-power/foundation-of-power-150px.webp",
    alt: "Foundations of Power",
    titles: { en: "Foundations of Power", pt: "Fundamentos do Poder", de: "Grundlagen der Macht", es: "Fundamentos del Poder", fr: "Fondements de la Puissance", ja: "パワーの基盤" },
    links: { en: "../../hero-wars-dominion-era/events/foundations-of-power-en.html", pt: "../../hero-wars-dominion-era/events/foundations-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Hero Power", pt: "Torneio de Poder do Herói", de: "Helden-Kraftturnier", es: "Torneo de Poder del Héroe", fr: "Tournoi de Puissance des Héros", ja: "ヒーローパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/hero-tournament-of-power-en.html", pt: "../../hero-wars-dominion-era/events/hero-tournament-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Secret Wealth", pt: "Riqueza Secreta", de: "Geheimer Reichtum", es: "Riqueza Secreta", fr: "Richesse Secrète", ja: "秘密の富" },
        links: { en: "#section3", pt: "#section3", de: "#section3", es: "#section3", fr: "#section3", ja: "#section3" }
      },
      {
        titles: { en: "Emeralds Sale x5", pt: "Promoção de Esmeraldas x5", de: "Smaragd-Angebot x5", es: "Oferta de Esmeraldas x5", fr: "Vente d'Émeraudes x5", ja: "エメラルドセール x5" },
        links: { en: "#section5", pt: "#section5", de: "#section5", es: "#section5", fr: "#section5", ja: "#section5" }
      }
    ]
  },
  {
    weekday: "Tuesday",
    date: "Jan, 06",
    image: "../../hero-wars-dominion-era/images/hero/aidan/aidan.webp",
    alt: "Aidan Brawls",
    titles: { en: "Aidan Brawls Best Teams", pt: "Melhores Times para Brawls do Aidan", de: "Aidan Brawls Beste Teams", es: "Mejores Equipos para Brawls de Aidan", fr: "Meilleurs Équipes Brawls Aidan", ja: "エイダン・ブロウル ベストチーム" },
    links: { en: "../../hero-wars-dominion-era/hero-brawls/aidan-brawls-en.html", pt: "../../hero-wars-dominion-era/hero-brawls/aidan-brawls-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Hero Brawls Event Missions", pt: "Missões do Evento de Brawls", de: "Helden-Brawls Missionen", es: "Misiones del Evento Brawls", fr: "Missions Événement Brawls", ja: "ヒーローブロウル イベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events/heroes-brawls-battles-en.html", pt: "../../hero-wars-dominion-era/events/heroes-brawls-battles-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Aidan Guide", pt: "Guia do Aidan", de: "Aidan Leitfaden", es: "Guía de Aidan", fr: "Guide Aidan", ja: "エイダン ガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/aidan-en.html", pt: "../../hero-wars-dominion-era/characters/aidan-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Wednesday",
    date: "Jan, 07",
    image: "../../hero-wars-dominion-era/images/hero/aidan/aidan.webp",
    alt: "Aidan Brawls",
    titles: { en: "Aidan Brawls Best Teams", pt: "Melhores Times para Brawls do Aidan", de: "Aidan Brawls Beste Teams", es: "Mejores Equipos para Brawls de Aidan", fr: "Meilleurs Équipes Brawls Aidan", ja: "エイダン・ブロウル ベストチーム" },
    links: { en: "../../hero-wars-dominion-era/hero-brawls/aidan-brawls-en.html", pt: "../../hero-wars-dominion-era/hero-brawls/aidan-brawls-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Hero Brawls Event Missions", pt: "Missões do Evento de Brawls", de: "Helden-Brawls Missionen", es: "Misiones del Evento Brawls", fr: "Missions Événement Brawls", ja: "ヒーローブロウル イベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events/heroes-brawls-battles-en.html", pt: "../../hero-wars-dominion-era/events/heroes-brawls-battles-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Aidan Guide", pt: "Guia do Aidan", de: "Aidan Leitfaden", es: "Guía de Aidan", fr: "Guide Aidan", ja: "エイダン ガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/aidan-en.html", pt: "../../hero-wars-dominion-era/characters/aidan-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Demon Dolls", pt: "Bonecas Demoníacas", de: "Dämonenpuppen", es: "Muñecas Demoníacas", fr: "Poupées Démoniaques", ja: "デーモンドール" },
        links: { en: "#section1", pt: "#section1", de: "#section1", es: "#section1", fr: "#section1", ja: "#section1" }
      }
    ]
  },
  {
    weekday: "Thursday",
    date: "Jan, 08",
    image: "../../hero-wars-dominion-era/images/hero/aidan/aidan.webp",
    alt: "Aidan Brawls",
    titles: { en: "Aidan Brawls Best Teams", pt: "Melhores Times para Brawls do Aidan", de: "Aidan Brawls Beste Teams", es: "Mejores Equipos para Brawls de Aidan", fr: "Meilleurs Équipes Brawls Aidan", ja: "エイダン・ブロウル ベストチーム" },
    links: { en: "../../hero-wars-dominion-era/hero-brawls/aidan-brawls-en.html", pt: "../../hero-wars-dominion-era/hero-brawls/aidan-brawls-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Hero Brawls Event Missions", pt: "Missões do Evento de Brawls", de: "Helden-Brawls Missionen", es: "Misiones del Evento Brawls", fr: "Missions Événement Brawls", ja: "ヒーローブロウル イベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events/heroes-brawls-battles-en.html", pt: "../../hero-wars-dominion-era/events/heroes-brawls-battles-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Aidan Guide", pt: "Guia do Aidan", de: "Aidan Leitfaden", es: "Guía de Aidan", fr: "Guide Aidan", ja: "エイダン ガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/aidan-en.html", pt: "../../hero-wars-dominion-era/characters/aidan-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Demon Dolls", pt: "Bonecas Demoníacas", de: "Dämonenpuppen", es: "Muñecas Demoníacas", fr: "Poupées Démoniaques", ja: "デーモンドール" },
        links: { en: "#section1", pt: "#section1", de: "#section1", es: "#section1", fr: "#section1", ja: "#section1" }
      }
    ]
  },
  {
    weekday: "Friday",
    date: "Jan, 09",
    image: "../../hero-wars-dominion-era/images/events/guardians-of-outland/guardians-of-outland-150px.webp",
    alt: "Guardians of Outland",
    titles: { en: "Guardians of Outland", pt: "Guardiões da Fronteira", de: "Wächter des Außenlands", es: "Guardianes de la Frontera", fr: "Gardiens de l'Outreterre", ja: "アウトランドの守護者" },
    links: { en: "../../hero-wars-dominion-era/events/guardians-of-outland-en.html", pt: "../../hero-wars-dominion-era/events/guardians-of-outland-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Piggy Bank", pt: "Porquinho", de: "Sparschwein", es: "Alcancía", fr: "Tirelire", ja: "ピギーバンク" },
        links: { en: "#section4", pt: "#section4", de: "#section4", es: "#section4", fr: "#section4", ja: "#section4" }
      }
    ]
  },
  {
    weekday: "Saturday",
    date: "Jan, 10",
    image: "../../hero-wars-dominion-era/images/events/area-of-conquest/area-of-conquest-150px.webp",
    alt: "Area of Conquest",
    titles: { en: "Area of Conquest", pt: "Área de Conquista", de: "Eroberungsgebiet", es: "Área de Conquista", fr: "Zone de Conquête", ja: "征服エリア" },
    links: { en: "../../hero-wars-dominion-era/events/area-of-conquest-en.html", pt: "../../hero-wars-dominion-era/events/area-of-conquest-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Guardians of Outland", pt: "Guardiões da Fronteira", de: "Wächter des Außenlands", es: "Guardianes de la Frontera", fr: "Gardiens de l'Outreterre", ja: "アウトランドの守護者" },
        links: { en: "../../hero-wars-dominion-era/events/guardians-of-outland-en.html", pt: "../../hero-wars-dominion-era/events/guardians-of-outland-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Piggy Bank", pt: "Porquinho", de: "Sparschwein", es: "Alcancía", fr: "Tirelire", ja: "ピギーバンク" },
        links: { en: "#section4", pt: "#section4", de: "#section4", es: "#section4", fr: "#section4", ja: "#section4" }
      }
    ]
  },
  {
    weekday: "Sunday",
    date: "Jan, 11",
    image: "../../hero-wars-dominion-era/images/events/area-of-conquest/area-of-conquest-150px.webp",
    alt: "Area of Conquest",
    titles: { en: "Area of Conquest", pt: "Área de Conquista", de: "Eroberungsgebiet", es: "Área de Conquista", fr: "Zone de Conquête", ja: "征服エリア" },
    links: { en: "../../hero-wars-dominion-era/events/area-of-conquest-en.html", pt: "../../hero-wars-dominion-era/events/area-of-conquest-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Guardians of Outland", pt: "Guardiões da Fronteira", de: "Wächter des Außenlands", es: "Guardianes de la Frontera", fr: "Gardiens de l'Outreterre", ja: "アウトランドの守護者" },
        links: { en: "../../hero-wars-dominion-era/events/guardians-of-outland-en.html", pt: "../../hero-wars-dominion-era/events/guardians-of-outland-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Secret Wealth", pt: "Riqueza Secreta", de: "Geheimer Reichtum", es: "Riqueza Secreta", fr: "Richesse Secrète", ja: "秘密の富" },
        links: { en: "#section3", pt: "#section3", de: "#section3", es: "#section3", fr: "#section3", ja: "#section3" }
      }
    ]
  },
  {
    weekday: "Monday",
    date: "Jan, 12",
    image: "../../hero-wars-dominion-era/images/events/area-of-conquest/area-of-conquest-150px.webp",
    alt: "Area of Conquest",
    titles: { en: "Area of Conquest", pt: "Área de Conquista", de: "Eroberungsgebiet", es: "Área de Conquista", fr: "Zone de Conquête", ja: "征服エリア" },
    links: { en: "../../hero-wars-dominion-era/events/area-of-conquest-en.html", pt: "../../hero-wars-dominion-era/events/area-of-conquest-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Secret Wealth", pt: "Riqueza Secreta", de: "Geheimer Reichtum", es: "Riqueza Secreta", fr: "Richesse Secrète", ja: "秘密の富" },
        links: { en: "#section3", pt: "#section3", de: "#section3", es: "#section3", fr: "#section3", ja: "#section3" }
      }
    ]
  },
  {
    weekday: "Tuesday",
    date: "Jan, 13",
    image: "../../hero-wars-dominion-era/images/events/elemental-synergy/elemental-synergy-150px.webp",
    alt: "Elemental Synergy",
    titles: { en: "Elemental Synergy", pt: "Sinergia Elemental", de: "Elementare Synergie", es: "Sinergia Elemental", fr: "Synergie Élémentaire", ja: "エレメンタルシナジー" },
    links: { en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Area of Conquest", pt: "Área de Conquista", de: "Eroberungsgebiet", es: "Área de Conquista", fr: "Zone de Conquête", ja: "征服エリア" },
        links: { en: "../../hero-wars-dominion-era/events/area-of-conquest-en.html", pt: "../../hero-wars-dominion-era/events/area-of-conquest-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emeralds Sale x4", pt: "Promoção de Esmeraldas x4", de: "Smaragd-Angebot x4", es: "Oferta de Esmeraldas x4", fr: "Vente d'Émeraudes x4", ja: "エメラルドセール x4" },
        links: { en: "#section5", pt: "#section5", de: "#section5", es: "#section5", fr: "#section5", ja: "#section5" }
      }
    ]
  },
  {
    weekday: "Wednesday",
    date: "Jan, 14",
    image: "../../hero-wars-dominion-era/images/events/seers-game/seers-game-150px.webp",
    alt: "Seer's Game",
    titles: { en: "Seer's Game", pt: "Jogo do Vidente", de: "Spiel des Sehers", es: "Juego del Vidente", fr: "Jeu du Voyant", ja: "予言者のゲーム" },
    links: { en: "../../hero-wars-dominion-era/events/seers-game-hwde-en.html", pt: "../../hero-wars-dominion-era/events/seers-game-hwde-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Elemental Synergy", pt: "Sinergia Elemental", de: "Elementare Synergie", es: "Sinergia Elemental", fr: "Synergie Élémentaire", ja: "エレメンタルシナジー" },
        links: { en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Portal Anomaly", pt: "Anomalia do Portal", de: "Portal-Anomalie", es: "Anomalía del Portal", fr: "Anomalie du Portail", ja: "ポータルアノマリー" },
        links: { en: "../../hero-wars-dominion-era/events/a-portal-anomaly-en.html", pt: "../../hero-wars-dominion-era/events/a-portal-anomaly-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      }
    ]
  },
  {
    weekday: "Thursday",
    date: "Jan, 15",
    image: "../../hero-wars-dominion-era/images/events/a-portal-anomaly/a-portal-anomaly-150px.webp",
    alt: "Portal Anomaly",
    titles: { en: "Portal Anomaly", pt: "Anomalia do Portal", de: "Portal-Anomalie", es: "Anomalía del Portal", fr: "Anomalie du Portail", ja: "ポータルアノマリー" },
    links: { en: "../../hero-wars-dominion-era/events/a-portal-anomaly-en.html", pt: "../../hero-wars-dominion-era/events/a-portal-anomaly-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Elemental Synergy", pt: "Sinergia Elemental", de: "Elementare Synergie", es: "Sinergia Elemental", fr: "Synergie Élémentaire", ja: "エレメンタルシナジー" },
        links: { en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Seer's Game", pt: "Jogo do Vidente", de: "Spiel des Sehers", es: "Juego del Vidente", fr: "Jeu du Voyant", ja: "予言者のゲーム" },
        links: { en: "../../hero-wars-dominion-era/events/seers-game-hwde-en.html", pt: "../../hero-wars-dominion-era/events/seers-game-hwde-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Demon Dolls", pt: "Bonecas Demoníacas", de: "Dämonenpuppen", es: "Muñecas Demoníacas", fr: "Poupées Démoniaques", ja: "デーモンドール" },
        links: { en: "#section1", pt: "#section1", de: "#section1", es: "#section1", fr: "#section1", ja: "#section1" }
      }
    ]
  },
  {
    weekday: "Friday",
    date: "Jan, 16",
    image: "../../hero-wars-dominion-era/images/events/a-portal-anomaly/a-portal-anomaly-150px.webp",
    alt: "Portal Anomaly",
    titles: { en: "Portal Anomaly", pt: "Anomalia do Portal", de: "Portal-Anomalie", es: "Anomalía del Portal", fr: "Anomalie du Portail", ja: "ポータルアノマリー" },
    links: { en: "../../hero-wars-dominion-era/events/a-portal-anomaly-en.html", pt: "../../hero-wars-dominion-era/events/a-portal-anomaly-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Seer's Game", pt: "Jogo do Vidente", de: "Spiel des Sehers", es: "Juego del Vidente", fr: "Jeu du Voyant", ja: "予言者のゲーム" },
        links: { en: "../../hero-wars-dominion-era/events/seers-game-hwde-en.html", pt: "../../hero-wars-dominion-era/events/seers-game-hwde-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Demon Dolls", pt: "Bonecas Demoníacas", de: "Dämonenpuppen", es: "Muñecas Demoníacas", fr: "Poupées Démoniaques", ja: "デーモンドール" },
        links: { en: "#section1", pt: "#section1", de: "#section1", es: "#section1", fr: "#section1", ja: "#section1" }
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Saturday",
    date: "Jan, 17",
    image: "../../hero-wars-dominion-era/images/events/tournament-of-titan-power/tournament-of-titan-power-150px.webp",
    alt: "Tournament of Titan Power",
    titles: { en: "Tournament of Titan Power", pt: "Torneio de Poder dos Titãs", de: "Titanenkraft-Turnier", es: "Torneo de Poder de Titanes", fr: "Tournoi de Puissance des Titans", ja: "タイタンパワートーナメント" },
    links: { en: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-en.html", pt: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Sunday",
    date: "Jan, 18",
    image: "../../hero-wars-dominion-era/images/events/power-of-the-titans/power-of-the-titans-150px.webp",
    alt: "Power of the Titans",
    titles: { en: "Power of the Titans", pt: "Poder dos Titãs", de: "Macht der Titanen", es: "Poder de los Titanes", fr: "Puissance des Titans", ja: "タイタンの力" },
    links: { en: "../../hero-wars-dominion-era/events/power-of-the-titans-en.html", pt: "../../hero-wars-dominion-era/events/power-of-the-titans-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Titan Power", pt: "Torneio de Poder dos Titãs", de: "Titanenkraft-Turnier", es: "Torneo de Poder de Titanes", fr: "Tournoi de Puissance des Titans", ja: "タイタンパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-en.html", pt: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Monday",
    date: "Jan, 19",
    image: "../../hero-wars-dominion-era/images/events/foundations-of-power/foundation-of-power-150px.webp",
    alt: "Foundations of Power",
    titles: { en: "Foundations of Power", pt: "Fundamentos do Poder", de: "Grundlagen der Macht", es: "Fundamentos del Poder", fr: "Fondements de la Puissance", ja: "パワーの基盤" },
    links: { en: "../../hero-wars-dominion-era/events/foundations-of-power-en.html", pt: "../../hero-wars-dominion-era/events/foundations-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Titan Power", pt: "Torneio de Poder dos Titãs", de: "Titanenkraft-Turnier", es: "Torneo de Poder de Titanes", fr: "Tournoi de Puissance des Titans", ja: "タイタンパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-en.html", pt: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Power of the Titans", pt: "Poder dos Titãs", de: "Macht der Titanen", es: "Poder de los Titanes", fr: "Puissance des Titans", ja: "タイタンの力" },
        links: { en: "../../hero-wars-dominion-era/events/power-of-the-titans-en.html", pt: "../../hero-wars-dominion-era/events/power-of-the-titans-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Tuesday",
    date: "Jan, 20",
    image: "../../hero-wars-dominion-era/images/events/foundations-of-power/foundation-of-power-150px.webp",
    alt: "Foundations of Power",
    titles: { en: "Foundations of Power", pt: "Fundamentos do Poder", de: "Grundlagen der Macht", es: "Fundamentos del Poder", fr: "Fondements de la Puissance", ja: "パワーの基盤" },
    links: { en: "../../hero-wars-dominion-era/events/foundations-of-power-en.html", pt: "../../hero-wars-dominion-era/events/foundations-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Titan Power", pt: "Torneio de Poder dos Titãs", de: "Titanenkraft-Turnier", es: "Torneo de Poder de Titanes", fr: "Tournoi de Puissance des Titans", ja: "タイタンパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-en.html", pt: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Power of the Titans", pt: "Poder dos Titãs", de: "Macht der Titanen", es: "Poder de los Titanes", fr: "Puissance des Titans", ja: "タイタンの力" },
        links: { en: "../../hero-wars-dominion-era/events/power-of-the-titans-en.html", pt: "../../hero-wars-dominion-era/events/power-of-the-titans-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Piggy Bank", pt: "Porquinho", de: "Sparschwein", es: "Alcancía", fr: "Tirelire", ja: "ピギーバンク" },
        links: { en: "#section4", pt: "#section4", de: "#section4", es: "#section4", fr: "#section4", ja: "#section4" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Wednesday",
    date: "Jan, 21",
    image: "../../hero-wars-dominion-era/images/events/foundations-of-power/foundation-of-power-150px.webp",
    alt: "Foundations of Power",
    titles: { en: "Foundations of Power", pt: "Fundamentos do Poder", de: "Grundlagen der Macht", es: "Fundamentos del Poder", fr: "Fondements de la Puissance", ja: "パワーの基盤" },
    links: { en: "../../hero-wars-dominion-era/events/foundations-of-power-en.html", pt: "../../hero-wars-dominion-era/events/foundations-of-power-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Tournament of Titan Power", pt: "Torneio de Poder dos Titãs", de: "Titanenkraft-Turnier", es: "Torneo de Poder de Titanes", fr: "Tournoi de Puissance des Titans", ja: "タイタンパワートーナメント" },
        links: { en: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-en.html", pt: "../../hero-wars-dominion-era/events/the-tournament-of-titan-power-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Piggy Bank", pt: "Porquinho", de: "Sparschwein", es: "Alcancía", fr: "Tirelire", ja: "ピギーバンク" },
        links: { en: "#section4", pt: "#section4", de: "#section4", es: "#section4", fr: "#section4", ja: "#section4" }
      },
      {
        titles: { en: "Emeralds Sale x4", pt: "Promoção de Esmeraldas x4", de: "Smaragd-Angebot x4", es: "Oferta de Esmeraldas x4", fr: "Vente d'Émeraudes x4", ja: "エメラルドセール x4" },
        links: { en: "#section5", pt: "#section5", de: "#section5", es: "#section5", fr: "#section5", ja: "#section5" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Thursday",
    date: "Jan, 22",
    image: "../../hero-wars-dominion-era/images/titans-hwde/mairi/mairi.webp",
    alt: "Mairi Titan Brawls",
    titles: { en: "Mairi Titan Brawls Best Teams", pt: "Melhores Times Brawls da Mairi", de: "Mairi Titan Brawls Beste Teams", es: "Mejores Equipos Brawls de Mairi", fr: "Meilleurs Équipes Brawls Mairi", ja: "マイリ タイタンブロウル ベストチーム" },
    links: { en: "../../hero-wars-dominion-era/titans-brawls/mairi-brawls-en.html", pt: "../../hero-wars-dominion-era/titans-brawls/mairi-brawls-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Titan Brawl Missions", pt: "Missões de Brawl de Titãs", de: "Titan Brawl Missionen", es: "Misiones de Brawl de Titanes", fr: "Missions Brawl Titan", ja: "タイタンブロウルミッション" },
        links: { en: "../../hero-wars-dominion-era/events/titan-brawl-en.html", pt: "../../hero-wars-dominion-era/events/titan-brawl-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Mairi Titan Guide", pt: "Guia da Titã Mairi", de: "Mairi Titan Leitfaden", es: "Guía del Titán Mairi", fr: "Guide Titan Mairi", ja: "マイリタイタンガイド" },
        links: { en: "../../hero-wars-dominion-era/titans/mairi-en.html", pt: "../../hero-wars-dominion-era/titans/mairi-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Friday",
    date: "Jan, 23",
    image: "../../hero-wars-dominion-era/images/titans-hwde/mairi/mairi.webp",
    alt: "Mairi Titan Brawls",
    titles: { en: "Mairi Titan Brawls Best Teams", pt: "Melhores Times Brawls da Mairi", de: "Mairi Titan Brawls Beste Teams", es: "Mejores Equipos Brawls de Mairi", fr: "Meilleurs Équipes Brawls Mairi", ja: "マイリ タイタンブロウル ベストチーム" },
    links: { en: "../../hero-wars-dominion-era/titans-brawls/mairi-brawls-en.html", pt: "../../hero-wars-dominion-era/titans-brawls/mairi-brawls-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Titan Brawl Missions", pt: "Missões de Brawl de Titãs", de: "Titan Brawl Missionen", es: "Misiones de Brawl de Titanes", fr: "Missions Brawl Titan", ja: "タイタンブロウルミッション" },
        links: { en: "../../hero-wars-dominion-era/events/titan-brawl-en.html", pt: "../../hero-wars-dominion-era/events/titan-brawl-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Mairi Titan Guide", pt: "Guia da Titã Mairi", de: "Mairi Titan Leitfaden", es: "Guía del Titán Mairi", fr: "Guide Titan Mairi", ja: "マイリタイタンガイド" },
        links: { en: "../../hero-wars-dominion-era/titans/mairi-en.html", pt: "../../hero-wars-dominion-era/titans/mairi-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Secret Wealth", pt: "Riqueza Secreta", de: "Geheimer Reichtum", es: "Riqueza Secreta", fr: "Richesse Secrète", ja: "秘密の富" },
        links: { en: "#section3", pt: "#section3", de: "#section3", es: "#section3", fr: "#section3", ja: "#section3" }
      },
      {
        titles: { en: "Fluffy's Preorder", pt: "Pré-venda do Fluffy", de: "Fluffy Vorbestellung", es: "Preventa de Fluffy", fr: "Précommande Fluffy", ja: "フラッフィー先行予約" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Saturday",
    date: "Jan, 24",
    image: "../../hero-wars-dominion-era/images/events/guardians-of-outland/guardians-of-outland-150px.webp",
    alt: "Guardians of Outland",
    titles: { en: "Guardians of Outland", pt: "Guardiões da Fronteira", de: "Wächter des Außenlands", es: "Guardianes de la Frontera", fr: "Gardiens de l'Outreterre", ja: "アウトランドの守護者" },
    links: { en: "../../hero-wars-dominion-era/events/guardians-of-outland-en.html", pt: "../../hero-wars-dominion-era/events/guardians-of-outland-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Titan Brawl Missions", pt: "Missões de Brawl de Titãs", de: "Titan Brawl Missionen", es: "Misiones de Brawl de Titanes", fr: "Missions Brawl Titan", ja: "タイタンブロウルミッション" },
        links: { en: "../../hero-wars-dominion-era/events/titan-brawl-en.html", pt: "../../hero-wars-dominion-era/events/titan-brawl-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Mairi Titan Brawls Best Teams", pt: "Melhores Times Brawls da Mairi", de: "Mairi Titan Brawls Beste Teams", es: "Mejores Equipos Brawls de Mairi", fr: "Meilleurs Équipes Brawls Mairi", ja: "マイリ タイタンブロウル ベストチーム" },
        links: { en: "../../hero-wars-dominion-era/titans-brawls/mairi-brawls-en.html", pt: "../../hero-wars-dominion-era/titans-brawls/mairi-brawls-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Mairi Titan Guide", pt: "Guia da Titã Mairi", de: "Mairi Titan Leitfaden", es: "Guía del Titán Mairi", fr: "Guide Titan Mairi", ja: "マイリタイタンガイド" },
        links: { en: "../../hero-wars-dominion-era/titans/mairi-en.html", pt: "../../hero-wars-dominion-era/titans/mairi-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Secret Wealth", pt: "Riqueza Secreta", de: "Geheimer Reichtum", es: "Riqueza Secreta", fr: "Richesse Secrète", ja: "秘密の富" },
        links: { en: "#section3", pt: "#section3", de: "#section3", es: "#section3", fr: "#section3", ja: "#section3" }
      },
      {
        titles: { en: "Fluffy's Event", pt: "Evento do Fluffy", de: "Fluffy Event", es: "Evento de Fluffy", fr: "Événement Fluffy", ja: "フラッフィーイベント" },
        labelOnly: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Sunday",
    date: "Jan, 25",
    image: "../../hero-wars-dominion-era/images/events/ascension-to-asgard/ascension-to-asgard-especial-invitation150px.webp",
    alt: "Ascension to Asgard",
    titles: { en: "Ascension to Asgard", pt: "Ascensão a Asgard", de: "Aufstieg nach Asgard", es: "Ascensión a Asgard", fr: "Ascension vers Asgard", ja: "アスガルドへの昇天" },
    links: { en: "../../hero-wars-dominion-era/events/ascension-to-asgard-en.html", pt: "../../hero-wars-dominion-era/events/ascension-to-asgard-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Guardians of Outland", pt: "Guardiões da Fronteira", de: "Wächter des Außenlands", es: "Guardianes de la Frontera", fr: "Gardiens de l'Outreterre", ja: "アウトランドの守護者" },
        links: { en: "../../hero-wars-dominion-era/events/guardians-of-outland-en.html", pt: "../../hero-wars-dominion-era/events/guardians-of-outland-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Monday",
    date: "Jan, 26",
    image: "../../hero-wars-dominion-era/images/events/the-great-storm/the-great-storm-150px.webp",
    alt: "The Great Storm",
    titles: { en: "The Great Storm", pt: "A Grande Tempestade", de: "Der Große Sturm", es: "La Gran Tormenta", fr: "La Grande Tempête", ja: "大嵐" },
    links: { en: "../../hero-wars-dominion-era/events/the-great-storm-en.html", pt: "../../hero-wars-dominion-era/events/the-great-storm-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Guardians of Outland", pt: "Guardiões da Fronteira", de: "Wächter des Außenlands", es: "Guardianes de la Frontera", fr: "Gardiens de l'Outreterre", ja: "アウトランドの守護者" },
        links: { en: "../../hero-wars-dominion-era/events/guardians-of-outland-en.html", pt: "../../hero-wars-dominion-era/events/guardians-of-outland-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Ascension to Asgard", pt: "Ascensão a Asgard", de: "Aufstieg nach Asgard", es: "Ascensión a Asgard", fr: "Ascension vers Asgard", ja: "アスガルドへの昇天" },
        links: { en: "../../hero-wars-dominion-era/events/ascension-to-asgard-en.html", pt: "../../hero-wars-dominion-era/events/ascension-to-asgard-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emerald Waterfalls", pt: "Cachoeiras de Esmeralda", de: "Smaragdwasserfälle", es: "Cascadas de Esmeraldas", fr: "Cascades d'Émeraudes", ja: "エメラルドの滝" },
        links: { en: "#section2", pt: "#section2", de: "#section2", es: "#section2", fr: "#section2", ja: "#section2" }
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Tuesday",
    date: "Jan, 27",
    image: "../../hero-wars-dominion-era/images/events/the-great-storm/the-great-storm-150px.webp",
    alt: "The Great Storm",
    titles: { en: "The Great Storm", pt: "A Grande Tempestade", de: "Der Große Sturm", es: "La Gran Tormenta", fr: "La Grande Tempête", ja: "大嵐" },
    links: { en: "../../hero-wars-dominion-era/events/the-great-storm-en.html", pt: "../../hero-wars-dominion-era/events/the-great-storm-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Ascension to Asgard", pt: "Ascensão a Asgard", de: "Aufstieg nach Asgard", es: "Ascensión a Asgard", fr: "Ascension vers Asgard", ja: "アスガルドへの昇天" },
        links: { en: "../../hero-wars-dominion-era/events/ascension-to-asgard-en.html", pt: "../../hero-wars-dominion-era/events/ascension-to-asgard-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Wednesday",
    date: "Jan, 28",
    image: "../../hero-wars-dominion-era/images/events/weaving-patterns/weaving-patterns-150px.webp",
    alt: "Weaving Patterns",
    titles: { en: "Weaving Patterns", pt: "Tecendo Padrões", de: "Webmuster", es: "Tejiendo Patrones", fr: "Tissage de Motifs", ja: "ウィービングパターン" },
    links: { en: "../../hero-wars-dominion-era/events/weaving-patterns-en.html", pt: "../../hero-wars-dominion-era/events/weaving-patterns-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "The Great Storm", pt: "A Grande Tempestade", de: "Der Große Sturm", es: "La Gran Tormenta", fr: "La Grande Tempête", ja: "大嵐" },
        links: { en: "../../hero-wars-dominion-era/events/the-great-storm-en.html", pt: "../../hero-wars-dominion-era/events/the-great-storm-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Emeralds Sale x4", pt: "Promoção de Esmeraldas x4", de: "Smaragd-Angebot x4", es: "Oferta de Esmeraldas x4", fr: "Vente d'Émeraudes x4", ja: "エメラルドセール x4" },
        links: { en: "#section5", pt: "#section5", de: "#section5", es: "#section5", fr: "#section5", ja: "#section5" }
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Thursday",
    date: "Jan, 29",
    image: "../../hero-wars-dominion-era/images/events/weaving-patterns/weaving-patterns-150px.webp",
    alt: "Weaving Patterns",
    titles: { en: "Weaving Patterns", pt: "Tecendo Padrões", de: "Webmuster", es: "Tejiendo Patrones", fr: "Tissage de Motifs", ja: "ウィービングパターン" },
    links: { en: "../../hero-wars-dominion-era/events/weaving-patterns-en.html", pt: "../../hero-wars-dominion-era/events/weaving-patterns-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Demon Dolls", pt: "Bonecas Demoníacas", de: "Dämonenpuppen", es: "Muñecas Demoníacas", fr: "Poupées Démoniaques", ja: "デーモンドール" },
        links: { en: "#section1", pt: "#section1", de: "#section1", es: "#section1", fr: "#section1", ja: "#section1" }
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "How to Get Fluffy 6★ F2P", pt: "Como Conseguir Fluffy 6★ F2P", de: "Fluffy 6★ F2P bekommen", es: "Cómo obtener Fluffy 6★ F2P", fr: "Comment obtenir Fluffy 6★ F2P", ja: "フラッフィー6★無課金入手方法" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-guide-pt.html", de: "", es: "", fr: "", ja: "" }
      },
      {
        titles: { en: "Fluffy Event Missions", pt: "Missões do Evento Fluffy", de: "Fluffy Event-Missionen", es: "Misiones del Evento Fluffy", fr: "Missions Événement Fluffy", ja: "フラッフィーイベントミッション" },
        links: { en: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-en.html", pt: "../../hero-wars-dominion-era/events-special-hwde/fluffy-event-missions-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Friday",
    date: "Jan, 30",
    image: "../../hero-wars-dominion-era/images/events/elemental-synergy/elemental-synergy-150px.webp",
    alt: "Elemental Synergy",
    titles: { en: "Elemental Synergy", pt: "Sinergia Elemental", de: "Elementare Synergie", es: "Sinergia Elemental", fr: "Synergie Élémentaire", ja: "エレメンタルシナジー" },
    links: { en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: [
      {
        titles: { en: "Weaving Patterns", pt: "Tecendo Padrões", de: "Webmuster", es: "Tejiendo Patrones", fr: "Tissage de Motifs", ja: "ウィービングパターン" },
        links: { en: "../../hero-wars-dominion-era/events/weaving-patterns-en.html", pt: "../../hero-wars-dominion-era/events/weaving-patterns-pt.html", de: "", es: "", fr: "", ja: "" },
        noStrip: true
      },
      {
        titles: { en: "Demon Dolls", pt: "Bonecas Demoníacas", de: "Dämonenpuppen", es: "Muñecas Demoníacas", fr: "Poupées Démoniaques", ja: "デーモンドール" },
        links: { en: "#section1", pt: "#section1", de: "#section1", es: "#section1", fr: "#section1", ja: "#section1" }
      },
      {
        titles: { en: "Fluffy Guide", pt: "Guia do Fluffy", de: "Fluffy Leitfaden", es: "Guía de Fluffy", fr: "Guide Fluffy", ja: "フラッフィーガイド" },
        links: { en: "../../hero-wars-dominion-era/characters/fluffy-en.html", pt: "../../hero-wars-dominion-era/characters/fluffy-pt.html", de: "", es: "", fr: "", ja: "" }
      }
    ]
  },
  {
    weekday: "Saturday",
    date: "Jan, 31",
    image: "../../hero-wars-dominion-era/images/events/elemental-synergy/elemental-synergy-150px.webp",
    alt: "Elemental Synergy",
    titles: { en: "Elemental Synergy", pt: "Sinergia Elemental", de: "Elementare Synergie", es: "Sinergia Elemental", fr: "Synergie Élémentaire", ja: "エレメンタルシナジー" },
    links: { en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: []
  },
  {
    weekday: "Sunday",
    date: "Feb, 01",
    image: "../../hero-wars-dominion-era/images/events/elemental-synergy/elemental-synergy-150px.webp",
    alt: "Elemental Synergy",
    titles: { en: "Elemental Synergy", pt: "Sinergia Elemental", de: "Elementare Synergie", es: "Sinergia Elemental", fr: "Synergie Élémentaire", ja: "エレメンタルシナジー" },
    links: { en: "../../hero-wars-dominion-era/events/elemental-synergy-en.html", pt: "../../hero-wars-dominion-era/events/elemental-synergy-pt.html", de: "", es: "", fr: "", ja: "" },
    noStrip: true,
    extra: []
  }
  
  // ===================== FEBRUARY 2026 =====================
  // Copy entries from January and change dates as needed
];
